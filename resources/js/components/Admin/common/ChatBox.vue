<template>
    <a-modal :visible="flagModalConfirm" @cancel="cancelModalConfirm" footer="" :closable="false" class="confirm-edit">
        <div class="row">
            <!-- DIRECT CHAT PRIMARY -->
                    ...
                    <div class="direct-chat-messages">
                        ...
                        <div class="direct-chat-msg">
                            <div class="direct-chat-info clearfix">
                                <span class="direct-chat-name pull-left">{{ userInfo.name }}</span>
                                <span class="direct-chat-timestamp pull-right">23 Jan 2:00 pm</span>
                            </div>
                            <!-- /.direct-chat-info -->
                            <img class="direct-chat-img" :src="`${userInfo.avatar}`"
...
export default {
    ...
    methods: {
        cancelModalConfirm() {
            this.$store.dispatch('toggleConfirmModal', false);
        },
        editProducts() {
            // Add logic for when the user clicks "OK"
            // After processing, close the modal
            this.$store.dispatch('toggleConfirmModal', false);
        },
        sendMessage() {
            // Example method to send message to BotMan
            // Implement axios call to BotMan endpoint here
            const formData = new FormData();
            formData.append("message", 'Hello BotMan!');
            formData.append("driver", 'web'),
            formData.append("userId", '1234')

            httpRequest
                .post('/api/botman', formData)
                .then((response) => {
                    console.log(response)
                    this.messages.push(response.data.messages);
                })
            this.userMessage = ''; // Clear message input after sending
        }
    },
    computed: {
        flagModalConfirm() {
